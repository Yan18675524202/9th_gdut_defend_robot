...
function [evalDB,trajDB] = Evaluation(x,Vr,goal,robot_A ,robot_B,pos,R,model,evalParam,area,sign)
evalDB = [];
trajDB = [];
...
dist    = CalcDistEval(xt,robot_A,robot_B,pos,R,area);    % 前项预测终点 距离最近障碍物的间隙得分 距离越远分数越高
vel     = abs(vt);                  % 速度得分 速度越快分越高
stopDist = CalcBreakingDist(vel,model); % 制动距离的计算
if dist > stopDist+0.7 % 如果可能撞到最近的障碍物 则舍弃此路径 （到最近障碍物的距离 大于 刹车距离 才取用）
